...
<template>
  ...
  closeable
  close-icon="close"
  ><div  v-for="item in cartList[shopId][activeIndex]"
  :key="item.id">
    <van-card
  :price="item.price"
  :desc="item.text"
...
const useCartEffect = (shopId, tabId) => {
  ...
  return { total, cartList, price, productList, arr, changeCartItemInfo }
}
...
export default {
  ...
  setup () {
    ...
    const { total, arr, cartList, price, productList, changeCartItemInfo, changeCartItemChecked } = useCartEffect(shopId, activeIndex)
    getItemData()
    return {
      value,
      shopId,
