...
<template>
  <div class="Goods">
    ...
    <div class="main">
      <p style="font-size:10px">{{cartList[0][0][0]["count"]}}</p>
      <van-card
        :num="total"
        price="2.00配送费"
          ...
          >
            <div v-if="activeIndex == item.id">
              ...
              >
                ...
                <template #footer>
                  <van-button v-if="it.count!==0" size="mini" @click="() => { changeCartItemInfo(shopId,item.id, it.id, it, -1)}">-</van-button>
                  <van-button size="mini">{{it.count || 0}}</van-button>
                  <van-button v-if="it.exit!==0" size="mini" @click="() => { changeCartItemInfo(shopId,item.id,it.id, it, 1) }">+</van-button>
                </template>
              </van-card>
            </div>
          </div>
...
const useCartEffect = (shopId, tabId) => {
  ...
  const total = computed(() => {
    // cartList[0][0][0]["count"]
    // {{cartList[0]["综合排序"][0]["count"]
    const shopList = cartList[shopId]
    let count = 0
    if (shopList) {
      for (const i in shopList) {
        const product = shopList[i][tabId]
