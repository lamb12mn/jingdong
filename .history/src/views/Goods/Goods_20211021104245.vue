          ...
          >
            <div v-if="activeIndex == item.id">
              ...
              >
                ...
                <template #footer>
                  <van-button v-if="it.count!==0" size="mini" @click="() => { changeCartItemInfo(shopId,activeIndex, it.id, it, -1),tab(shopId,activeIndex) }">-</van-button>
                  <van-button size="mini">{{it.count || 0}}</van-button>
                  <van-button v-if="it.exit!==0" size="mini" @click="() => { changeCartItemInfo(shopId,activeIndex,it.id, it, 1),tab(shopId,activeIndex) }">+</van-button>
                </template>
              </van-card>
            </div>
          </div>
...
export default {
  ...
  setup () {
    ...
    const tab(shopId,tabId)
    const { items, getItemData } = useClassifyEffect()
    const handelBackClick = useBackEffect()
    const { total, cartList, price, productList, changeCartItemInfo, changeCartItemChecked } = useCartEffect(shopId, tabId)
    getItemData()
    return {
      value,
      shopId,
