...
<template>
  <div class="Goods">
    ...
    <div class="main">
      <p style="font-size:10px">{{cartList[shopId]["综合排序"][0][count]}}</p>
      <van-card
        :num="total"
        price="2.00配送费"
          ...
          >
            <div v-if="activeIndex == item.id">
              ...
              >
                ...
                <template #footer>
                  <van-button v-if="it.count!==0" size="mini" @click="() => { changeCartItemInfo(shopId,item.text, it.id, it, -1)}">-</van-button>
                  <van-button size="mini">{{cartList[shopId][item.text][it.id][count]|| 0}}</van-button>
                  <van-button v-if="it.exit!==0" size="mini" @click="() => { changeCartItemInfo(shopId,item.text,it.id, it, 1) }">+</van-button>
                </template>
              </van-card>
            </div>
          </div>
  ...
  <div class="shopCart">
    <van-submit-bar :price="price * 100" button-text="提交订单" @submit="onSubmit(shopId)">
...
const useCartEffect = (shopId, tabId) => {
  ...
  const total = computed(() => {
    const productList = cartList[shopId][tabId]
    let count = 0
    if (productList) {
      for (const i in productList) {
        const product = productList[i][tabId]
  ...
  const price = computed(() => {
    const productList = cartList[tabId]
    let count = 0
    if (productList) {
      for (const i in productList) {
        const product = productList[i].list
