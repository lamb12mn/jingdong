...
<template>
              ...
              >
                ...
                <template #footer>
                  <van-button
                    ...
                    @click="
                      () => {
                        changeCartItemInfo(shopId, activeIndex, it.id, it, -1);
                      }
                    "
                    >-</van-button
                  >
                  <van-button size="mini">{{
                    cartList?.[shopId]?.[activeIndex]?.[it.id]?.count || 0
                  }}</van-button>
                  <van-button
                    v-if="it.exit !== 0"
                    size="mini"
  ...
  <div class="shopCart">
    <van-submit-bar
      :price="price * 100"
      button-text="提交订单"
      @submit="onSubmit(shopId, cartList)"
    >
      ...
      <van-action-bar-icon
        icon="star"
        :text="collected ? '已收藏' : '未收藏'"
        @click="
          () => {
            collected = !collected;
          }
        "
        :color="collected ? '#ff5000' : '#666'"
      />
      <template #tip>
    ...
    <van-popup
      ...
      ><div class="popupHeader">
        <van-checkbox v-model="allChecked"
          ><span @click="() => setCartItemsChecked(shopId)"
            >全选</span
          ></van-checkbox
        >
        <div class="p-H-right"></div>
        <div class="p-H-right" @click="() => cleanCartProducts(shopId)">
          清空购物车
        </div>
      </div>
      <van-checkbox-group v-model="checked" ref="checkboxGroup">
        <div v-for="item in productList" :key="item.id" style="display: flex">
            ...
            >
              ...
              <template #footer>
                ...
                <van-button
                  ...
                  @click="
                    () => {
                      changeCartItemInfo(shopId, activeIndex, item.id, item, 1);
                    }
                  "
                  >+</van-button
                >
...
const useCartEffect = (shopId, tabId) => {
  ...
  const price = computed(() => {
    ...
    if (productList) {
      for (const i in productList) {
        const product = productList[i]
        for (const j in product) {
          count += product[j].count * product[j].price
        }
      }
    }
  ...
  // 选择购物车中的内容
  const changeCartItemChecked = (shopId, tabId, productId) => {
    store.commit('changeCartItemChecked', { shopId, tabId, productId })
  }
  // 清空购物车
...
export default {
  ...
  setup () {
    ...
    const {
      ...
      cleanCartProducts,
      setCartItemsChecked
    } = useCartEffect(shopId, activeIndex)
    getItemData()
    return {
      checked,
      checkAll,
