      ...
      <div class="content">
        ...
        <div class="content-cards">
          <van-checkbox-group v-model="checked" ref="checkboxGroup">
            <van-checkbox v-for="(item,index) in cartGoods" :key="item.id" :name="index">
              <van-card
                :num="item.count"
                :price="item.price"
                :desc="item.text.slice(0,21)"
                title="商品标题"
                :thumb="item.img"
              >
                <template #tags>
                ...
                <template #footer>
                  <van-button v-if="itemShow" size="mini" @click="clearCount(event)">删除</van-button>
...
export default {
  ...
  setup () {
    ...
    const { onSubmit, onClickLink, handelBackClick } = useSubmitEffect()
    return {
      handelBackClick,
      checked,
      checkAll,
