      ...
      <div class="content">
        ...
        <div class="content-cards">
          <van-checkbox-group v-model="checked" ref="checkboxGroup">
            <van-checkbox style="width:100%" v-for="(item,index) in cartGoods" :key="item.id" :name="index">
              <van-card
                :num="item.count"
                :price="item.price"
...
const useSubmitEffect = () => {
  ...
  const clearCount = (id) => {
    // const { gooList } = localStorage
    // localStorage.removeItem(gooList[id])
    console.log(localStorage.gooList)
    console.log(id)
  }
  return {
    onSubmit,
